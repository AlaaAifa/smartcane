...
class _AlertsPageState extends State<AlertsPage> {
  ...
  Widget _buildAlertCard(Map<String, dynamic> alert) {
    ...
    final color = isSOS ? AppTheme.sosRed : AppTheme.helpYellow;
    final cin = alert["user_id"]?.toString();
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                                    ...
                                    _sectionCardFull(
                                      ...
                                      child: Column(
                                        children: [
                                          ...
                                          Row(
                                            children: [
                                              Expanded(child: _infoRowFull(Icons.badge_rounded, "CIN", currentAlert["user_id"]?.toString() ?? "Non renseign")),
                                              Expanded(child: _infoRowFull(Icons.cake_rounded, "ÂGE", "${user?['age'] ?? currentAlert['age'] ?? 'Non renseign'} ANS")),
                                            ],
                                          ),
                                          const Divider(height: 32, color: Color(0xFFF1F5F9)),
                                          Row(
                                            children: [
                                              Expanded(child: _infoRowFull(Icons.phone_rounded, "TÉLÉPHONE", user?["numero_de_telephone"]?.toString() ?? currentAlert["user_phone"]?.toString() ?? "Non renseign")),
                                              Expanded(child: _infoRowFull(Icons.contact_phone_rounded, "URGENCE", user?["contact_familial"]?.toString() ?? currentAlert["emergency_phone"]?.toString() ?? "Non renseign")),
                                            ],
                                          ),
                                        ],
                                      ),
                                ...
                                child: Column(
                                  children: [
                                    _sectionCardFull(
                                      ...
                                      child: Column(
                                        children: [
                                          _infoTileCompact("TYPE", currentAlert["type"]?.toString() ?? "Non renseign", color),
                                    ...
                                    _sectionCardFull(
                                      ...
                                      child: _statusTileFull(
                                        "ÉTAT PHYSIQUE", 
                                        liveStatus.toUpperCase(), 
                                        (liveStatus.contains("CHUTE") || liveState == 4) ? AppTheme.sosRed : AppTheme.neonGreen
                                      ),
                                    ),
                                  ],
                                ),
                  ...
                  _alertActions(currentAlert, user, ctx),
                ],
              ),
            ),
          );
  ...
  Widget _alertHeader(Map<String, dynamic> alert, bool isSOS, Color color) => Container(
    padding: const EdgeInsets.all(32),
    decoration: BoxDecoration(
    ...
    child: Row(
      children: [
        Container(
          ...
          child: Icon(isSOS ? Icons.emergency_rounded : Icons.help_rounded, color: color, size: 28),
        ),
        const SizedBox(width: 20),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("ALERTE ${alert['type']} DÉTECTÉE", style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 20, letterSpacing: 0.5)),
            const SizedBox(height: 4),
            Text("ID TRANSACTION: ${alert['alert_id']}", style: const TextStyle(color: Color(0xFF94A3B8), fontSize: 11, fontWeight: FontWeight.w800, letterSpacing: 1)),
          ],
        ),
        const Spacer(),
  ...
  Widget _alertActions(Map<String, dynamic> alert, Map<String, dynamic>? user, BuildContext ctx) => Container(
    padding: const EdgeInsets.all(32),
    ...
    child: Row(
      children: [
        Expanded(child: AppGradientButton(label: "APPELER CONTACTS", icon: Icons.phone_in_talk_rounded, color: AppTheme.primary, onTap: () => _showCallPicker(context, user, alert))),
        const SizedBox(width: 16),
        Expanded(
  ...
  Widget _statusTileFull(String label, String value, Color color) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 16), 
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(color: Color(0xFF64748B), fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5)), const SizedBox(height: 4), Text(value, style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 15))]))
        ]
      )
    );
  }
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(24),
          border: Border.all(color: _hovered ? widget.color.withOpacity(0.5) : const Color(0xFFF1F5F9), width: 2),
          boxShadow: [
            BoxShadow(
        ...
        child: Row(
          children: [
            widget.isSOS ? const _PulseIcon() : Container(
              ...
              child: Icon(Icons.help_outline_rounded, color: widget.color, size: 32),
            ),
            const SizedBox(width: 24),
            Expanded(
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      Container(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(color: widget.color, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 1)
                        ),
                      ),
                      const SizedBox(width: 12),
                  ...
                  const SizedBox(height: 12),
                  Text(widget.userName, style: const TextStyle(fontWeight: FontWeight.w900, color: AppTheme.primary, fontSize: 18)),
                  const SizedBox(height: 4),
                  Row(
                    children: [
                      const Icon(Icons.location_on_rounded, size: 14, color: Color(0xFF94A3B8)),
                      const SizedBox(width: 6),
                      Text("COORD: ${widget.alert['latitude']}, ${widget.alert['longitude']}", style: const TextStyle(color: Color(0xFF64748B), fontSize: 12, fontWeight: FontWeight.w600)),
                    ],
                  ),
                ],
              ),
            ...
            Container(
              ...
              child: Text(widget.timeStr, style: const TextStyle(color: AppTheme.primary, fontSize: 14, fontWeight: FontWeight.w800)),
            ),
          ],
        ),
      ),
...
class _PulseIconState extends State<_PulseIcon> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (context, child) => Container(
      padding: const EdgeInsets.all(16), 
      decoration: BoxDecoration(
        color: AppTheme.sosRed.withOpacity(0.1 + (0.15 * _c.value)), 
        borderRadius: BorderRadius.circular(16), 
        boxShadow: [
          BoxShadow(color: AppTheme.sosRed.withOpacity(0.3 * _c.value), blurRadius: 15 * _c.value, spreadRadius: 2 * _c.value)
        ]
      ), 
      child: const Icon(Icons.emergency_rounded, color: AppTheme.sosRed, size: 32)
    )
  );
}
