...
class _LoginPageState extends State<LoginPage> {
  ...
  void _verifyCode() async {
    ...
    setState(() { _isLoading = true; _error = null; });
    final success = await ApiService.verifyOtp(email, code);
    setState(() => _isLoading = false);
    ...
    if (success) {
      ...
      setState(() => _error = "Code incorrect ou serveur injoignable.");
    }
  }
  ...
  void _resetPasswordFinal() async {
    ...
    setState(() { _isLoading = true; _error = null; });
    final success = await ApiService.resetPassword(email, newPass);
    setState(() => _isLoading = false);
  ...
  Widget _buildForgotOTPForm() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text("Entrez le code envoyé sur votre e-mail", style: TextStyle(color: Colors.grey.shade600, fontSize: 16)),
