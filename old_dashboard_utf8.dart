...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _editUser(Map<String, dynamic> user) {
    ...
    final List<String> availablePathologies = [
      "Diab├¿te", "Hypertension", "Maladie cardiaque", "├ëpilepsie", 
      "Troubles de lÔÇÖ├®quilibre / Vertiges", "Difficult├® de mobilit├®", 
      "Baisse auditive", "Allergies m├®dicamenteuses", 
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("Informations G├®n├®rales", style: TextStyle(fontWeight: FontWeight.bold, color: AppTheme.primary)),
                  ...
                  const Text("Informations M├®dicales", style: TextStyle(fontWeight: FontWeight.bold, color: AppTheme.primary)),
                  const SizedBox(height: 16),
                  ...
                  if (pathologies["Allergies m├®dicamenteuses"]!) ...[
                    const SizedBox(height: 12),
                    _buildEditField("Pr├®ciser le(s) m├®dicament(s)", allergyCtrl),
                  ],
                  if (pathologies["Autre"]!) ...[
                    const SizedBox(height: 12),
                    _buildEditField("Pr├®ciser l'autre pathologie", otherCtrl),
                  ],
                  const SizedBox(height: 16),
                  Row(
                    children: [
                  ...
                  const SizedBox(height: 12),
                  _buildEditField("Observations compl├®mentaires", obsCtrl, isMultiline: true),
                ],
              ),
            ),
          ),
                ...
                if (res["success"]) {
                  if (mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("Utilisateur mis ├á jour avec succ├¿s"), backgroundColor: AppTheme.normalGreen),
                    );
                  }
                } else {
                  if (mounted) {
